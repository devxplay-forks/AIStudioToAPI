...
- `-v /path/to/data:/app/data`：挂载数据持久化目录（`/app/data/usage-stats.jsonl` 和 `/app/data/state.json`）
...
| 变量名                          | 描述                                                                                                                                                                | 默认值    |
| :------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :-------- |
| `INITIAL_AUTH_INDEX`            | 可选的启动身份验证索引覆盖。未设置时会从 `data/state.json` 恢复上次使用的账号。                                                                                     | 未设置    |
...
> 从旧版本升级时，如果现有 `.env` 中保留了 `INITIAL_AUTH_INDEX=0`，服务会继续固定从账号 `0` 启动。删除或注释该配置后，才会启用上次使用账号恢复。
